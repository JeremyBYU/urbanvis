...
function convertCell(coord, toCell = true, meta=MAZE_META) {
  if (toCell) {
    ...
    let j = bound(Math.floor((xMeters - meta.xres / 2) / meta.xres), 0, meta.ncols - 1)
    let i = bound(Math.floor((yMeters - meta.yres / 2) / meta.yres), 0, meta.rows - 1)
    let k = bound(Math.floor((zMeters - meta.zres / 2) / meta.zres), 0, meta.nslices - 1)
...
function createCircle(data, color = 0x808080) {
...
function createPlanner(map, start, goal=GOAL_SPHERICAL) {
  ...
  createCircle({x: goalCell[0], y: goalCell[1], z: goalCell[2]}, 0x008000)
  AsyncPlanner = AsyncAStar.util.createPlanner(map, startCell, goalCell, true, 'euclidean')
}
...
function plan() {
  ...
  if (scope.parameters.planner && scope.parameters.planner.active && AsyncPlanner) {
    if (!AsyncPlanner.finished) {
      let result = AsyncPlanner.searchAsync(1, () => null, (node) => createCircle(node.data))
      console.log(result)
    }
...
function createPlanner(map, start, goal, allowDiag = true, heuristic = 'manhattan') {
    ...
    const planner = new asyncastar_1.AsyncAstar(startNode, goalNode, hash, genSuccessorsPartial, manhattan, stopFn);
    return planner;
}
